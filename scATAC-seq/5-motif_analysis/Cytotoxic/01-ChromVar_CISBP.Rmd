...
Naive_RM <- DARS(ident.1 = "Naive CD8 T", 
                       ident.2 = "RM CD8 T")
Naive_RM$motif <- row.names(Naive_RM)

Naive_Tf <- DARS(ident.1 = "Naive CD8 T", 
                       ident.2 = "CD8 Tf")
Naive_Tf$motif <- row.names(Naive_Tf)

RM_Tf <- DARS(ident.1 = "RM CD8 T", 
              ident.2 = "CD8 Tf")
RM_Tf$motif <- row.names(RM_Tf)

DT::datatable(Naive_RM)
DT::datatable(Naive_Tf)
DT::datatable(RM_Tf)
```
...
p1 <- FeaturePlot(seurat,
      order = TRUE,
      features = "ENSG00000140968-LINE2752-IRF8-D-N2",
      min.cutoff = 'q1',
      max.cutoff = 'q99',
      pt.size = 0.8)  + scale_color_gradientn( colours = c('grey90', 'chartreuse3'))


p2 <-FeaturePlot(seurat,
      order = TRUE,
      features = "ENSG00000131196-LINE3215-NFATC1-D-N1",
      min.cutoff = 'q1',
      max.cutoff = 'q99',
      pt.size = 0.8) + scale_color_gradientn( colours = c('grey90', 'chartreuse3'))
