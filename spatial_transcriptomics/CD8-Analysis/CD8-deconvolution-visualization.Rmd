...
title: "CD8 deconvolution assessment"
editor_options: 
...
# Load SPOTlight data
spotlight_ls <- "{cd8}/{robj_dir}/spotlight_ls_cd8.rds" %>%
  glue::glue() %>% 
  here::here() %>%
  readRDS(file = .)

# Load names df data
nm_df <- "{cd8}/{robj_dir}/cd8_nm_df.rds" %>%
  glue::glue() %>% 
  here::here() %>%
  readRDS(file = .)
```
...
lapply(names(sp_obj@images), function(nm) {
  ...
  "{cd8}/{plt_dir}/cell_type_location_cd4_{nm_donor}_new.pdf" %>%
    glue::glue() %>%
    here::here() %>%
    cowplot::save_plot(
      filename = .,
...
"{cd8}/{plt_dir}/strat_bplot_{donor_id}.pdf" %>%
  glue::glue() %>%
  here::here() %>%
  cowplot::save_plot(
    filename = .,
...
lapply(Images(sp_obj), function(i) {
    ...
    "{cd8}/{plt_dir}/cor-mtrx_cell-type_{donor_id}.pdf" %>%
      glue::glue() %>%
      here::here() %>%
      cowplot::save_plot(
        filename = .,
...
"{cd8}/{plt_dir}/cor-mtrx_cell-type_integrated.pdf" %>%
  glue::glue() %>%
  here::here() %>%
  cowplot::save_plot(
    filename = .,
