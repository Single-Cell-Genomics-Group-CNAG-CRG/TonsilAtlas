...
cd8_vec <- c("Naive CD8 T", "SCM CD8 T", "CM CD8 T", "IFN CD8 T", "CD8 Tf", 
  "CXCR6+ RM CD8 T", "RM CD8 T", "DC recruiters CD8 T", "Nksig CD8 T",
  "CXCR6pn RM CD8 T", "Naive-CM CD8 T")
```
...
sc_obj@meta.data <- sc_obj@meta.data %>%
  dplyr::mutate(
    ...
    annotation_deconv = dplyr::if_else(
      annotation_20220215 %in% cd8_vec,
...
# Remove Cytotoxic since all the "Cytotoxic" cells are already in lvl5
table(sc_obj@meta.data$annotation_deconv, sc_obj@meta.data$annotation_level_1)
sc_obj <- sc_obj[, ! sc_obj$annotation_deconv %in% c("preBC", "preTC", "Cytotoxic")]

table(sc_obj@meta.data$annotation_deconv)

# level 5 CD8 T Cells
exist <- cd8_vec %in% unique(sc_obj$annotation_deconv)
names(exist) <- cd8_vec
exist
table(exist)
lvl5_cd8_obj <- sc_obj[, sc_obj$annotation_deconv %in% cd8_vec]
```
...
```{r}
markers_lvl1_out <- "{cd8}/{robj_dir}/markers_lvl1_cd8.RDS" %>% 
  glue::glue() %>% 
  here::here()
...
Now find the markers for CD8 T cells level 5 only between CD8 T cells
```{r}
markers_lvl5_out <- "{cd8}/{robj_dir}/markers_lvl5_cd8.RDS" %>% 
  glue::glue() %>% 
  here::here()
...
if (file.exists(markers_lvl5_out)) {
  ...
  #### Extract marker genes from each cluster ####
  Seurat::Idents(object = lvl5_cd8_obj) <- lvl5_cd8_obj@meta.data[, "annotation_deconv"]
...
markers_lvl1_sub <- markers_lvl1 %>%
    ...
    dplyr::filter(cluster != "Cytotoxic")
```

Add general CD8 T cell markers to the specific cell types
```{r}
  ...
  function(i) {
    markers_lvl5 %>%
      ...
      dplyr::bind_rows(markers_lvl1 %>%
          filter(cluster == "Cytotoxic" & avg_log2FC > 1.5)) %>%
...
## Save data
```{r}
decon_fn <- "{cd8}/{robj_dir}/spotlight_ls_cd8.rds" %>%
  glue::glue() %>% 
  here::here()
...
saveRDS(
  ...
  file = decon_fn)
# } else {
#   spotlight_ls <- readRDS(file = here::here(glue::glue("{cd8}/{robj_dir}/spotlight_ls_cd4.rds")))
# }
...
"{cd8}/{plt_dir}/cell_type_location_cd8_{donor_id}.pdf" %>%
  glue::glue() %>%
  here::here() %>%
  cowplot::save_plot(
    filename = .,
