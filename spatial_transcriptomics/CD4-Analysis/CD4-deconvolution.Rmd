...
## Introduction
In this script we are going to map  cd4 cell subtypes onto the Visium slides.
...
cd4_vec <- c(
  "Tfh-LZ-GC", "Naive", "GC-Tfh-SAP", "CM Pre-non-Tfh", "GC-Tfh-0X40",
  "CM PreTfh", "T-Trans-Mem", "T-Eff-Mem", "GC-Tf-regs", "Tfh-Mem",
  "T-helper", "Eff-Tregs", "Tfh T:B border", "non-GC-Tf-regs", "cycling T",
  "CM PreTfh/Pre-non-Tfh")
```
...
sc_obj@meta.data <- sc_obj@meta.data %>%
  dplyr::mutate(
    ...
    annotation_deconv = dplyr::if_else(
      annotation_20220215 %in% cd4_vec,
      as.character(annotation_20220215), as.character(annotation_level_1)),
    annotation_deconv = dplyr::case_when(
      annotation_deconv %in% c("CM Pre-non-Tfh", "CM PreTfh") ~ "CM PreTfh/Pre-non-Tfh",
...
# Remove cd4 since all the "cd4" cells are already in lvl5
sc_obj <- sc_obj[, ! sc_obj$annotation_deconv %in% c("preBC", "preTC", "CD4_T")]

# level 5 cd4 T Cells
exist <- cd4_vec %in% unique(sc_obj$annotation_deconv)
names(exist) <- cd4_vec
exist
table(exist)
lvl5_cd4_obj <- sc_obj[, sc_obj$annotation_deconv %in% cd4_vec]
```
...
```{r}
markers_lvl1_out <- "{cd4}/{robj_dir}/markers_lvl1_cd4.RDS" %>% 
  glue::glue() %>% 
  here::here()
...
Now find the markers for cd4 cells level 5 only between them
```{r}
markers_lvl5_out <- "{cd4}/{robj_dir}/markers_lvl5_cd4.RDS" %>% 
  glue::glue() %>% 
  here::here()
...
if (file.exists(markers_lvl5_out)) {
  ...
  #### Extract marker genes from each cluster ####
  Seurat::Idents(object = lvl5_cd4_obj) <- lvl5_cd4_obj@meta.data[, "annotation_deconv"]
...
markers_lvl1_sub <- markers_lvl1 %>%
    ...
    dplyr::filter(cluster != "CD4_T")
```

Add general cd4 cell markers to the specific cell types
```{r}
  ...
  function(i) {
    markers_lvl5 %>%
      ...
      dplyr::bind_rows(markers_lvl1 %>%
          filter(cluster == "CD4_T" & avg_log2FC > 1.5)) %>%
...
## Save data
```{r}
decon_fn <- "{cd4}/{robj_dir}/spotlight_ls_cd4.rds" %>%
  glue::glue() %>% 
  here::here()
...
saveRDS(
  ...
  file = decon_fn)
# } else {
#   spotlight_ls <- readRDS(file = here::here(glue::glue("{cd4}/{robj_dir}/spotlight_ls_cd4.rds")))
# }
