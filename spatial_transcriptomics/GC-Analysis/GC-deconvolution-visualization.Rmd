...
title: "GC deconvolution assessment"
editor_options: 
...
# Load SPOTlight data
spotlight_ls <- "{gct}/{robj_dir}/spotlight_ls_gc.rds" %>%
  glue::glue() %>% 
  here::here() %>%
  readRDS(file = .)

# Load names df data
nm_df <- "{gct}/{robj_dir}/gc_nm_df.rds" %>%
  glue::glue() %>% 
  here::here() %>%
  readRDS(file = .)
```
...
cols_gcbc_rna <- c(
  ...
  "DZ_LZ transition" = "#D4F0A3", #C5889C #D39285
  ...
  "Precursor MBCs" = "#E68098",
  "Reactivated proliferative MBCs" = "#CA4A68",  
...
cols_gcbc_atac <- c(
  ...
  "DZ G2Mphase" = "#6F66A3",
  "DZ Sphase" =  "#006FC4",
  "DZ_LZ transition"  = "#4EB2FF",
  "DZ non proliferative" = "#356200",
  "LZ" = "#FFF59A", 
  "prePC" = "#FECFC7" ,
  "LZ-DZ-re-entry" = "#E0EFEF",
  "MBC" = "#E68098")
...
new_cn <- data.frame(mod_nm = colnames(decon_mtrx)) %>%
  ...
  dplyr::mutate(
    plt_nm = dplyr::if_else(is.na(plt_nm), as.character(mod_nm), as.character(plt_nm))) %>%
  dplyr::distinct() %>%
  dplyr::pull(plt_nm)
...
Subset follicles only
```{r}
sp_obj <- sp_obj[, sp_obj$annotation_20220215 %in% c("Germinal Center", "Proliferating Follicle")]
```
...
lapply(names(sp_obj@images), function(nm) {
  ...
  "{gct}/{plt_dir}/cell_type_location_gc_{nm_donor}_new.pdf" %>%
    glue::glue() %>%
    here::here() %>%
    cowplot::save_plot(
      filename = .,
...
"{gct}/{plt_dir}/strat_bplot_{donor_id}.pdf" %>%
  glue::glue() %>%
  here::here() %>%
  cowplot::save_plot(
    filename = .,
...
lapply(Images(sp_obj), function(i) {
    ...
    "{gct}/{plt_dir}/cor-mtrx_cell-type_{donor_id}.pdf" %>%
      glue::glue() %>%
      here::here() %>%
      cowplot::save_plot(
        filename = .,
...
"{gct}/{plt_dir}/cor-mtrx_cell-type_integrated.pdf" %>%
  glue::glue() %>%
  here::here() %>%
  cowplot::save_plot(
    filename = .,
