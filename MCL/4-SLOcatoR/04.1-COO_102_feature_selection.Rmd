---
title: "Cell-of-origin prediction: Feature Selection (102)"
...
path_to_save <- here::here("MCL/results/R_objects/7.merged_tumoral_reference_query_102.rds")
path_to_save_query <- here::here("MCL/results/R_objects/M102_scRNAseq_seurat_object.rds")
path_to_save_df <- here::here("MCL/results/R_objects/dataframe_CD4_T_label_transfer_102.rds")
path_to_save_shiny_metadata <- here::here("MCL/results/R_objects/M102_scRNAseq_metadata_shiny.rds")
path_to_save_shiny_expression <- here::here("MCL/results/R_objects/M102_scRNAseq_expression_shiny.rds")
...
openxlsx::write.xlsx(
  markers_list,
  file = here::here("MCL/results/tables/markers_clusters_tumoral_cells_M102.xlsx")
)
```
...
# library(scuttle)
# reference <- subset(reference, assay == "3P")
# ref_sce <- as.SingleCellExperiment(reference)
# logcounts(ref_sce) <- reference[["RNA"]]@data
# summed <- aggregateAcrossCells(
#   ref_sce,
#   id = colData(ref_sce)[, "names_level_5_clusters_delta"]
# )
# logcounts(summed) <- normalizeCounts(summed, log = TRUE)
# summed_seu <- as.Seurat(summed)
# summed_seu <- FindVariableFeatures(summed_seu, nfeatures = 1000)
# reference <- integrate_assays(
#   reference,
#   assay_specific = TRUE,
#   shared_hvg = VariableFeatures(summed_seu),
#   assay_var = "gem_id"
# )
# reference <- RunUMAP(reference, dims = 1:30, reduction = "harmony")
```
