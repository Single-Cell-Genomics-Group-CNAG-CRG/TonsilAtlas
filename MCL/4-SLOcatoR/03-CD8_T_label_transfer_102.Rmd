---
title: "Transfer label CD8 T, NK and ILC cells (102)"
...
In this notebook we will transfer the label and UMAP coordinates from the CD8 T cells, NK and ILC of the tonsil atlas to the cytotoxic cells of the MCL sample (case 102) using SLOcatoR.
...
path_to_save1 <- here("MCL/results/R_objects/7.merged_CD8_T_ILC_NK_reference_query_102.rds")
path_to_save2 <- here("MCL/results/R_objects/7.seurat_CD8_T_ILC_NK_102_annotated.rds")
...
Thus, we will conserve clusters 0 and 4. However, we know that naive CD8 T cells can cluster with naive CD4 T cells, so we will fetch and merge them:

```{r}
cd4 <- subset(query, idents = c("1", "2"))
FeaturePlot(cd4, c("CD8A", "NOSIP"), order = TRUE)
...
DimPlot(cd4, group.by = "naive_CD8")
Idents(cd4) <- "naive_CD8"
...
# Merge both
query <- subset(query, idents = c("0", "4"))
naive_cd8 <- subset(cd4, idents = "1_3")
query <- merge(x = query, y = naive_cd8)
```
