---
title: 'Map myeloid cells (experiments 2 & 3 tonsil, deep annotation)'
...
```{r}
path_to_slan <- here("scRNA-seq/results/R_objects/slancytes_revision/1.3.seurat_slan_cells_tonsil_classified_level_1_experiments_2_and_3.rds")
path_to_myeloid <- here("scRNA-seq/results/R_objects/final_clusters/20220215/20220215_myeloid_seurat_obj.rds")


# Misc
label_var <- "annotation_20220619"
batch_var <- "type"
...
# Remove lingering doublets/poor-quality cells
...
DimPlot(query)
markers <- FindAllMarkers(query, logfc.threshold = 1, only.pos = TRUE)
DT::datatable(markers, options = list(scrollX = TRUE))
DimPlot(query)
```

Cluster 2 is composed of T-cells, cluster 3 of keratinocytes/poor-quality cells:

```{r}
query <- query[, !(query$seurat_clusters %in% c("2", "3"))]
DimPlot(query)
```
...
if (all(colnames(query) == annotation_query_df$query_cells)) {
  query$annotation_20220619 <- annotation_query_df$annotation
  query$annotation_probability <- annotation_query_df$annotation_prob
}
DimPlot(query, group.by = "annotation_20220619", cols = colors_myel)
...
```{r}
saveRDS(query, here("scRNA-seq/results/R_objects/slancytes_revision/3.1.seurat_slan_cells_tonsil_classified_deep_annotation_experiments_2_and_3.rds"))
```
