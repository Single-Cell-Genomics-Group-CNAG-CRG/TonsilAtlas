...
do
 	echo $cell_type;
 	mkdir -p log/${cell_type}
 	mkdir -p log/${cell_type}/rf
	for res in ${resolutions[@]}; do
		echo $res
		sbatch --reservation=massoni-reserv --err "log/${cell_type}/rf/rf_${res}.err" --out "log/${cell_type}/rf/rf_${res}.log" -J "${cell_type}_${res}" -c 4 --time 15:00:00 --wrap "module purge; module load gcc/6.3.0 hdf5/1.10.1 R/3.6.0; echo [`date "+%Y-%m-%d %T"`] starting job on $HOSTNAME; Rscript ./01-fit_random_forest.R ${cell_type} ${res}; echo [`date "+%Y-%m-%d %T"`] job finished" 
	done
done
