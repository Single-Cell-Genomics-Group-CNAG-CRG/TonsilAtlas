...
```{r}
# Paths
path_to_lisi_scores <- here::here("scRNA-seq/2-QC/5-batch_effect_correction/3-data_integration_multiome/tmp/lisi_scores.rds")
...
path_to_save_supp_df <- here::here("data/raw_data_figures/lisi_scores_rna.csv")
```
...
lisi_scores_ggs <- purrr::map2(confounders, plot_titles, function(x, title) {
  p <- lisi_scores %>%
    ggplot(aes_string(
      ...
      scale_fill_manual(values = c("gray60", "limegreen")) +
...
assay_gg <- assay_df %>% 
  ...
  ggplot(aes(assay, lisi, color = assay, fill = assay)) +
    geom_violin() +
    stat_summary(fun = median, geom = "point", size = 1.25, color= "black") +
...
Clearly, the intermixing of cells in the 5' and multiome assays improves greatly after integration.


# Save

We will save the LISI scores to plot the supplementary figures for publication:

```{r}
confounders_df <- confounders_df[confounders_df$hospital != "Royal London", ]
lisi_scores <- lisi_scores[str_detect(rownames(lisi_scores), "^B", negate = TRUE), ]
confounders_df$barcode <- rownames(confounders_df)
colnames(lisi_scores) <- ifelse(
  colnames(lisi_scores) == "is_integrated",
  "is_integrated",
  str_c("lisi", colnames(lisi_scores), sep = "_")
)
lisi_scores$barcode <- str_sub(rownames(lisi_scores), 1, 36)
supp_lisi_df <- left_join(x = lisi_scores, y = confounders_df, by = "barcode")
write_delim(supp_lisi_df, path_to_save_supp_df, delim = ";", col_names = TRUE)
```
