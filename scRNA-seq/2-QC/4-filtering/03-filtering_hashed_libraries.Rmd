---
title: 'Quality Control: Hashed cells'
...
```{r}
# Paths
path_to_merged <- here::here("scRNA-seq/results/R_objects/seurat_hashed_merged.rds")
...
path_to_save_obj <- here::here("scRNA-seq/results/R_objects/seurat_hashed_merged_filtered.rds")
...
tonsil_hashed <- readRDS(path_to_merged)
tonsil_hashed
...
```{r}
# Calculate percentage of mitochondrial expression
tonsil_hashed$pct_mt <- PercentageFeatureSet(
  tonsil_hashed,
  pattern = "^MT-"
)
...
```{r}
Idents(tonsil_hashed) <- "HTO_classification.global"
tonsil_hashed <- subset(tonsil_hashed, idents = c("Singlet", "Doublet"))
tonsil_hashed
```
...
```{r}
# Calculate mean SNR
snr_means_df <- tonsil_hashed@meta.data %>%
...
snr_gg <- tonsil_hashed@meta.data %>%
...
n_features_means_df <- tonsil_hashed@meta.data %>%
  dplyr::group_by(library_name) %>%
  dplyr::summarise(
...
n_features_gg <- tonsil_hashed@meta.data %>%
  dplyr::filter(HTO_classification.global == "Singlet") %>%
    horizontal_boxplot(
    categorical_var = "library_name",
...
```{r fig.wide=TRUE}
lib_size_hist <- tonsil_hashed@meta.data %>%
...
```{r fig.wide=TRUE}
n_genes_hist1 <- tonsil_hashed@meta.data %>%
...
```{r}
pct_mt_hist <- tonsil_hashed@meta.data %>%
...
```{r}
tonsil_hashed$pct_ribosomal <- PercentageFeatureSet(
  tonsil_hashed,
  pattern = "^RPS"
)
pct_ribosomal_hist <- tonsil_hashed@meta.data %>%
...
n_genes_vs_lib_size <- FeatureScatter(
  ...
  pt.size = 0.15,
  cols = rep("black", length(levels(Idents(tonsil_hashed))))
)
...
```{r}
metadata_before_qc <- tonsil_hashed@meta.data
is_low_quality <- 
  tonsil_hashed$nCount_RNA < min_lib_size |
  tonsil_hashed$nFeature_RNA < min_n_genes |
  tonsil_hashed$pct_mt > max_pct_mt
table(is_low_quality)
tonsil_hashed$keep_cells <- !is_low_quality
Idents(tonsil_hashed) <- "keep_cells"
tonsil_hashed <- subset(tonsil_hashed, idents = TRUE)
metadata_after_qc <- tonsil_hashed@meta.data
```
...
```{r}
tonsil_hashed$has_high_lib_size <- 
  tonsil_hashed$nCount_RNA > max_lib_size |
  tonsil_hashed$nFeature_RNA > max_n_genes
table(tonsil_hashed$has_high_lib_size)
```
...
```{r fig.wide=TRUE}
n_cells <- Matrix::rowSums(tonsil_hashed[["RNA"]]@counts > 0)
gene_qc <- n_cells %>% 
  as.data.frame() %>% 
  ggplot(aes(n_cells)) + 
...
```{r}
kept_genes <- rownames(tonsil_hashed)[n_cells > min_cells]
table(n_cells > min_cells)
tonsil_hashed <- subset(tonsil_hashed, features = kept_genes)
tonsil_hashed
```
